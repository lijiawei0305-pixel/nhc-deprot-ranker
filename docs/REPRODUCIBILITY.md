...
Phase 0/1/2/3 utilities use no quantum-chemistry package. Phase 1 uses pandas and PyArrow for normalized Parquet output; Phases 2–3 add SciPy statistics, joblib serialization, scikit-learn-compatible estimators, and headless Matplotlib reports.
...
The large processed, baseline-result, and hierarchical-result artifacts are intentionally ignored. Phase 1 evidence is recorded in `PROCESSED_V001_MANIFEST.json`; B0/B1 evidence is recorded in `BASELINES_V001_MANIFEST.json`; H1 model, OOF, nested search, bootstrap, metric, and figure hashes are recorded in `HIERARCHICAL_V001_MANIFEST.json`. Full-pool prediction hashes remain unavailable because Phase 5 has not started.
