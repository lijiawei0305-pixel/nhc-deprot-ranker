...
| `pyscf_may_start` / `PySCFHandoffReceipt` (`phase9b_handoff.py:269-289`, handoff validators later in file) | an in-process result opens PySCF | structural/byte identity primitives | replace admission authority with cross-process receipt plus independent supervisor and A2 reads | handoff v1 and tamper matrix |
